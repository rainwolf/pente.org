...
public class MoveTreeModel extends DefaultTreeModel implements TreeSelectionListener {
    ...
    private void notifyListeners(PlunkNode n) {
        ...
        treeGridState.clear();
        if (puzzle != null) addPuzzleMovesToTree();
    ...
    public void setPuzzle(Puzzle puzzle) {
        this.puzzle = puzzle;
        addPuzzleMovesToBoard();
        addPuzzleMovesToTree();
    }

    public void addPuzzleMovesToTree() {
        for (int m : puzzle.getWhiteMoves()) {
            treeGridState.setPosition(m, 1);
        }
        for (int m : puzzle.getBlackMoves()) {
            treeGridState.setPosition(m, 2);
        }
    }

    public void addPuzzleMovesToBoard() {
        for (int m : puzzle.getWhiteMoves()) {
            boardGridState.setPosition(m, 1);
        }
        for (int m : puzzle.getBlackMoves()) {
            boardGridState.setPosition(m, 2);
        }
    }
    ...
    public void firstMove() {
        ...
        if (treeGridState.getNumMoves() > 1) {
            ...
            treeGridState.clear();
            if (puzzle != null) addPuzzleMovesToTree();
            else treeGridState.addMove(180);
