...
import org.pente.gameServer.client.web.CoordinatesListPanel;
import org.pente.gameServer.client.web.GridBoardOrderedPieceCollectionAdapter;
import org.pente.gameServer.client.web.PenteBoardCanvas;
import org.pente.gameServer.client.web.PenteBoardOrderedPieceCollectionAdapter;
...
public class GameBoard extends Container implements GridCoordinatesChangeListener,
    ...
    public GameBoard() {
        init(getDefaultGameOptions(), getDefaultCoordinates(), null, null, null, false);
    }

    public GameBoard(CoordinatesListPanel coordinatesList) {
        init(getDefaultGameOptions(), getDefaultCoordinates(), null,
                coordinatesList, null, false);
    }

    public GameBoard(CoordinatesListPanel coordinatesList,
                     PenteBoardComponent board) {
        GameOptions go = getDefaultGameOptions();
        if (board instanceof GameOptionsChangeListener) {
            ((GameOptionsChangeListener) board).gameOptionsChanged(go);
        }
        init(go, getDefaultCoordinates(), null,
                coordinatesList, board, false);
    }
    ...
    public GameBoard(PenteBoardComponent board,
        ...
        init(go, getDefaultCoordinates(), null,
                null, board, allowMovesWhileViewingHistory);
    }


    public GameBoard(GameOptions gameOptions,
                     GridCoordinates coordinates,
                     GridBoardListener gridBoardListener, //optional
                     CoordinatesListPanel coordinatesList,
                     boolean allowMovesWhileViewingHistory) { //optional
        init(gameOptions, coordinates, gridBoardListener,
                coordinatesList, null, allowMovesWhileViewingHistory);
    }
    ...
    private void init(
        ...
        if (board != null) {
            ...
            gridBoardComponent = penteCanvas;
        }
        ...
        if (coordinatesList != null) {
            coordinatesList.addOrderedPieceCollectionVisitListener(gridBoard);
        }
        ...
        PenteStatePieceCollectionAdapter dkeryoState = new PenteStatePieceCollectionAdapter(getGomoku());
        ...
        GoStatePieceCollectionAdapter go9State = new GoStatePieceCollectionAdapter(9, 9);
        go9State.addOrderedPieceCollectionListener(gridBoard);

        GoStatePieceCollectionAdapter go13State = new GoStatePieceCollectionAdapter(13, 13);
        go13State.addOrderedPieceCollectionListener(gridBoard);


        if (coordinatesList != null) {
            penteState.addOrderedPieceCollectionListener(coordinatesList);
            keryoState.addOrderedPieceCollectionListener(coordinatesList);
            gpenteState.addOrderedPieceCollectionListener(coordinatesList);
            dpenteState.addOrderedPieceCollectionListener(coordinatesList);
            gomokuState.addOrderedPieceCollectionListener(coordinatesList);
            poofPenteState.addOrderedPieceCollectionListener(coordinatesList);
            ...
            go9State.addOrderedPieceCollectionListener(coordinatesList);
            go13State.addOrderedPieceCollectionListener(coordinatesList);
        }
