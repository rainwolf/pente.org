...
<% }
      ...
      Scanned activity log file for <b><%= date %></b>.<br>
      <a href="viewLog.jsp?date=<%= date %>">View the whole log file</a><br>
	  Found <b><font color="red"><%= events.size() %></font></b> suspicious events.<br>
...
<%	  for (LogData d : events) {
		  ...
		  if (susp.equals("2: ")) {
			  ...
			  <a href="viewLog.jsp?date=<%= date %>&s=<%= d.getStartLine() %>&l=<%= d.getLine() %>#l">Line <%= d.getLine() %></a>
			  <%= linked %><br>
<%		  } else { %>
              <a href="viewLog.jsp?date=<%= date %>&s=<%= d.getStartLine() %>&l=<%= d.getLine() %>#l">Line <%= d.getLine() %></a>
	          <%= s %><br>
...
<table border="1" width="700">
 ...
 <tr>
   <td colspan="2">Click the line number next to events to view the event in
    context with the rest of the log file.</td>
 </tr>
 <tr>
...
in at the same time for example.</td>
 </tr>
 <tr>
   <td>Game match</td>
