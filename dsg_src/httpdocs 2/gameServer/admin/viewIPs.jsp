...
<% }
       ...
       <h3>Viewing <%= vn ? "IPs" : "Names" %> for <%= vn ? name : ip %></h3>
       <table width="700"><tr><td>Keep in mind that IPs change frequently for some players, and some IPs
       ...
       help sometimes in seeing if there is cheating.</td></tr></table><br>
...
<% 		for (Data d : uniqDataSorted) { %>
   		  <tr>
   		    <td>
   		    <% if (vn) { %>
   		       <a href="viewIPs.jsp?ip=<%= d.text %>"><%= d.text %></a><%
   		       } else { %>
   		       <a href="viewIPs.jsp?name=<%= d.text %>"><%= d.text %></a>
   		       <% } %>    
   		    </td>
   		    <td><%= df.format(d.access) %></td>
   		    <% if (!vn) { %>
