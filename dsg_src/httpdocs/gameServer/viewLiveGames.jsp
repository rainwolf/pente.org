...
<font size="-2">
   ...
   for (int i = 0; i < totalGames / 100 + 1; i++) {
      if (i == startNum) { %><%= i + 1 %><% } else { %><a
   href="/gameServer/viewLiveGames?p=<%= dsgPlayerData.getName() %>&g=<%= game %>&t=<%= myTotal %>&count=<%= totalGames %>&w=<%= myWins %>&s=<%= i*100 %>"><%= i + 1 %>
</a> <% } %>
   <% } %>
   ]</font>
<% }
...
-->
      ...
      <td><% PlayerData d = g.getPlayer1Data(); %>
         <%@include file="vgplayerLink.jspf" %>
      </td>
