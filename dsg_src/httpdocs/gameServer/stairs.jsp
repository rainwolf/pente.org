...
<table>
   <tr>
      <td valign="top">
         ...
         Pente.org now has a fully automated King of the Hill. The format is similar to ladders or stairs that may be
         familiar from other sites.
         Each game has a hill and each hill has steps. Everyone starts out at the bottom step, you can advance one step
         higher by winning a rated set,
         if you lose the set and are not already on the bottom step, then you drop a step. A draw results in no change.
         Emtpy steps are removed as
         they appear, if there are only 2 players on 2 steps, and the higher placed player keeps beating the lower
         placed player, nothing changes.
         <br>
         <br>
         A few more rules
         <ul>
            <li>The player on the top step is king of the hill and gets a <img src="/gameServer/images/kothcrown.gif">
               if they occupy the top step alone, the more hills you conquer, the darker your crown becomes:
               <% for (int i = 1; i < 40; i++) { %>
               <img src="/gameServer/images/kothcrown<%=i%>.png">
               <%}%></li>
            <li>non-subscribers who don't play any King of the Hill games for 62 days are removed from the hill,
               subscribers have 100 days leeway,
            </li>
            <li>non-subscribers can only participate in one hill, <a href="https://pente.org/gameServer/subscriptions">subscribers</a>
               can participate in all hills,
            </li>
         ...
         <ul>
            <li>non-subscribers can only play 2 sets at any given time, this includes active sets, sent invitations, and
               invitations you haven't responded to yet,
            </li>
            <li>non-subscribers who don't play any King of the Hill games for 31 days are removed from the hill, for
               subscribers, this limit is set at 100 days,
            </li>
            <li><a href="https://pente.org/gameServer/subscriptions">subscribers</a> can play unlimited KotH games, when
               <a href="https://pente.org/gameServer/subscriptions">subscribers</a> have 5 or more ongoing KotH sets,
               including waiting invitations, they cannot be challenged anymore, but can still send out challenges,
            </li>
            <li>if your opponent leaves the hill before the set is over, the result still counts towards your hill
               position,
            </li>
            <li>joining or leaving a hill, and challenging other players has to be done from this page or your mobile
               device,
            </li>
            <li>declining an invitation affects your position if the timeout is 5 days or more, and, you are king of the
               hill or your rating is higher than that of the challenger.
            </li>
         </ul>
         The challenge button appears when you can challenge a player, and they're also marked with a green background.
         This button takes you to a page where you can decide on the timeout and post an invitation from there.
         <br>
         <br>
 ...
 -->
                  ...
                  <td>
                     ...
                     <form name="joinLeaveForm" method="post" action="/gameServer/koth" style="margin:0;padding:0;">
                        ...
                        <a class="boldbuttons" href="javascript: submitJoinLeaveForm()"
                           style="margin-right:6px; margin-left: 6px"><span>Leave this hill</span></a>
                     </form>
                     <%
                     } else {
                     %>
                     <form name="joinLeaveForm" method="post" action="/gameServer/koth" style="margin:0;padding:0;">
                        ...
                        <a class="boldbuttons" href="javascript: submitJoinLeaveForm()"
                           style="margin-right:6px; margin-left: 6px"><span>Join this hill</span></a>
                     </form>
                     <%
                        if (!dsgPlayerData.hasPlayerDonated()) { %>
                     <u><b>note:</b></u> you will be automatically removed from any other hills you are in when you
                     join.
                     <% }
                     }
                     %>
                  ...
                  <td>
                     <form name="new_game_form" method="post"
                           action="<%= request.getContextPath() %>/gameServer/tb/newKotH.jsp">
                        <input type="hidden" name="invitee" value="">
                        <input type="hidden" name="game" value="<%=game%>">
                        <!-- <input type="submit" name="send open hill invitation" value="send open hill invitation"> -->
                        <a class="boldbuttons" href="javascript: submitNewGameForm()"
                           style="margin-right:6px; margin-left: 6px"><span>Send Open Hill Invitation</span></a>
                     </form>
                  </td>
            ...
            <table border="1" width="450">
               <tr>
                  ...
                  <td colspan="<%=(game > 50)?4:3%>" align="center" bgcolor="#ff8105">
                     <font color="white">
                        <h1><%=(game > 50 ? "Turn-based " : "") + GridStateFactory.getGameName(game) + " (" + (hill != null ? hill.getNumPlayers() : 0) + ")"%>
                        </h1></font>
...
//			steps.get(i).getPlayers().sort((o1, o2) -> o1.getLastGame().compareTo(o2.getLastGame()));
               ...
               <tr>
                  <td bgcolor="<%=steps.get(i).hasPlayer(myPid)?"#ff8105":"#2C862F"%>"
                      colspan="<%=(iAmMember && game > 50)?4:3%>" align="center"><font
                     color="white"><b><%=(i == steps.size() - 1) ? "top of the hill" : "Step " + (i + 1)%>
                  </b></font></td>
               </tr>
               <%
               ...
               <tr <%=(canIchallenge && myPid != pid && (myStep - i) * (myStep - i) < 5 && canChallengeThem) ? "bgcolor=\"#deecde\"" : ""%>>
                  ...
                  <td align="center">
                     ...
                     %>
                     <form id="new_game_form_<%=d.getName()%>" name="new_game_form_<%=d.getName()%>" method="post"
                           action="<%= request.getContextPath() %>/gameServer/tb/newKotH.jsp">
         ...
         <ul>
            <li>On Januari 29th, 2018,
               <% if (true) {
                  DSGPlayerData d = dsgPlayerStorer.loadPlayer("dmitriking"); %>
               <%@ include file="playerLink.jspf" %>
               <%}%>
               won all 27 KotH crowns.
            </li>
         </ul>
         ...
         <table border="1" cellpadding="2" cellspacing="0" bordercolor="black">
            <tr>
               <td><a href="profile?viewName=dmitriking">dmitriking</a> <img src="/gameServer/images/kothcrown27.png">
               </td>
            </tr>
            <tr>
      ...
      <td width="29%" valign="top">
         ...
         <table align="right">
            ...
            <tr <%=(color % 2 == 0) ? "bgcolor=\"#deecde\"" : ""%>>
               <td align="right">
                  <a href="/gameServer/stairs.jsp?game=<%=gameInt%>"><b>
                     <h2><%="TB-" + GridStateFactory.getGameName(gameInt)%> (<%=hill.getNumPlayers()%>)</h2></b></a>
                  <%
...
//				DSGPlayerGameData dsgPlayerGameData = d.getPlayerGameData(CacheKOTHStorer.tbGames[i]);
            ...
            <tr <%=(color % 2 == 0) ? "bgcolor=\"#deecde\"" : ""%>>
               <td align="right">
                  <a href="/gameServer/stairs.jsp?game=<%=gameInt%>"><b><h2><%=GridStateFactory.getGameName(gameInt)%>
                     (<%=hill.getNumPlayers()%>)</h2></b></a>
                  <%
                     long kingPid = hill.getKing();
                     if (kingPid != 0) {
                        DSGPlayerData d = dsgPlayerStorer.loadPlayer(kingPid);
