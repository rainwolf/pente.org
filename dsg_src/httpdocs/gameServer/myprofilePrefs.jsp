...
<form enctype="multipart/form-data"
   ...
   <table width="100%" border="0" colspacing="0" colpadding="0">
      ...
      <tr>
         <td>
            <input type="checkbox" name="personalizeAds" value="Y" <% if (personalizeAds) { %>checked<% } %>>Allow
            personalized ads<br>
         </td>
      </tr>
      <% } %>
      ...
      <tr>
         <td>
            <input type="checkbox" name="email" value="Y" <% if (email) { %>checked<% } %>>Email me a copy of DSG
            messages I receive<br>
            <input type="checkbox" name="emailSent" value="Y" <% if (emailSent) { %>checked<% } %>>Email me a copy of
            DSG messages I send
         </td>
      </tr>
      <tr>
      ...
      <tr>
         <td><br>
            ...
            You get <%=MySQLDSGPlayerStorer.FLOATINGVACATIONDAYS%> floating vacation days each year, and you have <font
               color="red"><%=floatingHourDays / 24%> days and <%=floatingHourDays % 24%> hours</font> left for
            <b><%=Calendar.getInstance().get(Calendar.YEAR)%>
            </b>.
            (These vacation days do not roll over to <%=Calendar.getInstance().get(Calendar.YEAR) + 1%>.)

            <br>
            <br>
            These vacation days are floating so you don't have to do anything, which is perfect for vacation. They are
            automatically used whenever a game is about to time out, when that happens, and you still have vacation days
            left, 1 hour is subtracted and timeout of the game that's about to expire is extended by that hour. If
            adding an hour from your vacation days tumbles you into a weekend day, then the timeout is extended to the
            end of that weekend day.
            <br>
            <br>
            Do you want even more vacation days, then we offer <a href="/gameServer/subscriptions">amazing vacation
               deals!</a>.
      ...
      <tr>
         <td>
            ...
            messages (both game room chat and through Pente.org website) from a player change the checkboxes below and
            save. To ignore a
      ...
      <tr>
         <td>
            <table border="1" cellpadding="3" cellspacing="0" bordercolor="gray">
               ...
               <tr>
                  <td><b><a href="../profile?viewName=<%= ignored.getName() %>"><%= ignored.getName() %>
                  </a></b></td>
                  <td><input type="checkbox"
                             name="<%= i.getIgnorePid() %>_invite"<%= i.getIgnoreInvite() ? " checked" : "" %>
                             value="Y"></td>
                  <td><input type="checkbox"
                             name="<%= i.getIgnorePid() %>_chat"<%= i.getIgnoreChat() ? " checked" : "" %> value="Y">
                  </td>
               </tr>
               <% }
               } %>
