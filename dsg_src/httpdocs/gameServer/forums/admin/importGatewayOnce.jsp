...
<p>
   <font size="-1"><b>Importing...</b></font>
<p>
<ul>
   <font size="-1">
      ...
      Jive is currently importing messages into forum <i><%=forum.getName()%>
   </i> from account <%= ((Pop3Importer) importer).getUsername() %>@<%= ((Pop3Importer) importer).getHost() %>  <br>
      <% } else if (importer instanceof NewsgroupImporter) { %>
      Jive is currently importing messages into forum <i><%=forum.getName()%>
   </i> from newsgroup <%= ((NewsgroupImporter) importer).getNewsgroup() %>  <br>
      <% } %>
      <% if (startTime != -1) {%>
