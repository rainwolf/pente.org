...
<p>
   ...
   <font size="-1"><i>Message Approved.</i>
      <p></font>

      <%  } else if (msgreject) { %>

   <font size="-1"><i>Message Rejected.</i>
      <p></font>
...
<table cellpadding="0" cellspacing="0" border="0" width="100%">
   <tr>
      <td>
         ...
         <b>Pending Message: <%= message.getSubject() %>
         </b>
         <% } else { %>
         <b>Pending Thread: <%= message.getSubject() %>
         </b>
         <% } %>
      </td>
      <td align="right">
...
<table cellpadding="2" cellspacing="0" border="0">
   <tr>
      ...
      <td><font size="-1"><a
         href="pending.jsp?approve=true&forum=<%= forum.getID() %>&thread=<%= thread.getID() %>&message=<%= message.getID() %>"
      ...
      <td><font size="-1"><a
         href="pending.jsp?reject=true&forum=<%= forum.getID() %>&thread=<%= thread.getID() %>&message=<%= message.getID() %>"
...
<form action="pending.jsp">
   ...
   <table cellpadding="2" cellspacing="0" border="0">
      <tr>
         ...
         <td><input type="text" name="subject" value="<%= StringUtils.escapeHTMLTags(message.getUnfilteredSubject()) %>"
                    size="65" maxlength="200"></td>
      </tr>
      <tr>
         <td valign="top"><font size="-1">Message</font></td>
         <td>
            <textarea name="body" cols="60" rows="8"
                      wrap="virtual"><%= StringUtils.escapeHTMLTags(message.getUnfilteredBody()) %></textarea>
         </td>
      </tr>
      <tr>
