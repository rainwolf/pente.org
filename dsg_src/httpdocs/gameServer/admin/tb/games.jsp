...
<% for (TBGame game : games) { 
	   ...
	   if (p1 != null) { p1g = p1.getPlayerGameData(game.getGame()); }
       ...
       <tr>
         ...
         <td><% if (dsgPlayerGameData != null) { %><%@ include file="../../ratings.jspf" %><% } %></td>
         <% dsgPlayerGameData = p2g; %>
         <td><% if (p2 != null) { %><a href="player.jsp?pid=<%= game.getPlayer2Pid() %>"><%= p2.getName()%></a><% } %></td>
         <td><% if (dsgPlayerGameData != null) { %><%@ include file="../../ratings.jspf" %><% } %></td>
         <td><%= game.getNumMoves() + 1 %></td>
         <td><%= game.getDaysPerMove() %></td>
         <td><% if (game.getState() == TBGame.STATE_ACTIVE) { %><%= Utilities.getTimeLeft(game.getTimeoutDate().getTime()) %><% } %></td>
         <td><%= game.isRated() ? "Rated" : "Not Rated" %></td>
       </tr>
<% } %>
