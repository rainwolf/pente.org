function openwin2(plugin, port, width, height, guest) {
   url = '/gameServer/set-announce.jsp?plugin=' + plugin + '&port=' + port +
      '&size=' + width;
   if (guest) {
      url = url + '&guest=' + guest;
   }
...
function handlePlay2(plugin, size, guest) {
   ...
   openwin2(plugin, port, width, height, guest);
}
