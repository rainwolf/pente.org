...
<table border="0" cellpadding="1" cellspacing="0">
   ...
   <tr>
      ...
      <td><%= tourney.getInitialTime() %> days per move
         (<%= "Vacation is " + (tourney.getIncrementalTime() > 0 ? "" : "<b> not </b>") + "available for this tournament" %>
         )
      </td>
      <% } else { %>
      <td><%= tourney.getInitialTime() %> minutes initial /
         <%= tourney.getIncrementalTime() %> seconds incremental
      </td>
      <% } %>
   </tr>
   <tr>
      ...
      <td>
         ...
         Rating must be <b>below <%= restriction.getValue() %>
      </b>.<br>
         <% } else if (restriction.getType() == Restriction.RATING_RESTRICTION_ABOVE) { %>
         Rating must be <b>above <%= restriction.getValue() %>
      </b>.<br>
         <% } else if (restriction.getType() == Restriction.GAMES_RESTRICTION_ABOVE) { %>
         Must play at least <b><%= restriction.getValue() %>
      </b> games.<br>
         <% }
         }
         } %>
   ...
   <tr>
      <td>Signup ends:</td>
      <td><%= dateTimeFormat.format(tourney.getSignupEndDate()) %>
      </td>
   </tr>
   <tr>
      <td>Play starts:&nbsp;&nbsp;&nbsp;</td>
      <td><%= dateTimeFormat.format(tourney.getStartDate()) %>
      </td>
   </tr>
   <% if (tourney.isComplete()) { %>
   <tr>
      <td>Completion date:&nbsp;&nbsp;&nbsp;</td>
      <td><%= dateTimeFormat.format(tourney.getEndDate()) %>
      </td>
   </tr>
   <tr>
      <td><b>Winner:</b>&nbsp;&nbsp;&nbsp;</td>
