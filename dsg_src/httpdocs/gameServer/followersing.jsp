...
This page provides an overview of players you follow and players who follow you. Following someone means you subscribe to updates from that player. There are no updates implemented at the moment, but first one to be implemented is being able to notify followers that you are available to play in the live game room. Non-subscribers can follow up to <%= followingLimit %> players,
<a href="https://pente.org/gameServer/subscriptions">subscribers</a> are not limited.
...
<center>
   ...
   <form name="new_game_form" method="post"
      ...
      <select size="1" id="game" name="game" onchange="this.form.submit()">
         ...
         <option <% if (game == games[i].getId()) { %>selected <% } %>
                 value="<%= games[i].getId() %>"><%= GridStateFactory.getDisplayName(games[i].getId()) %>
         </option>
         <%--
      ...
      <br>
      <label>
         <input id="allow_followers_be_notified" <%=(allow_followers_be_notified ? "checked" : "")%>
                onchange="this.form.submit()" name="allow_followers_be_notified" type="checkbox" value="true"/>
         Allow followers be notified when I appear in a live game room.
      </label>
      <br>
      <label>
         <input
            id="allow_notification_online_from_following" <%=(allow_notification_online_from_following ? "checked" : "")%>
            onchange="this.form.submit()" name="allow_notification_online_from_following" type="checkbox" value="true"/>
         Send me a notification when people I follow enter a live game room <br> (at most 1 notification per person per
         10 minutes.)
      </label>
      <br>
   </form>
