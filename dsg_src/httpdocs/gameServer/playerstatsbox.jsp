      ...
      <td>
        ...
        for (int i = 0; i < games.length; i++) {
            ...
            <tr>
               ...
               <td><%= nf.format(dsgPlayerGameData.getWins()) %>
               </td>
               <td><%= nf.format(dsgPlayerGameData.getLosses()) %>
               </td>
               <td><%= dsgPlayerGameData.getDraws() %>
               </td>
               <td><%= nf.format(dsgPlayerGameData.getTotalGames()) %>
               </td>
               <td><%= (dsgPlayerGameData.getWins() + dsgPlayerGameData.getLosses() == 0 ? "" : profileNF.format(dsgPlayerGameData.getPercentageWins())) %>
               </td>
               <td><%= (dsgPlayerGameData.getStreak() > -1 ? "&nbsp;" : "") + dsgPlayerGameData.getStreak() %>
               </td>
               <td><%= dateFormat.format(dsgPlayerGameData.getLastGameDate()) %>
               </td>
            </tr>
            <% } %>
         </table>
